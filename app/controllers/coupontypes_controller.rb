class CoupontypesController < ApplicationController
  ...
  # fetch all records
  def index
    @o_all = Coupontype.search(params[:search]).order(sort_column + " " + sort_direction).paginate(:per_page => 5, :page => params[:page])
  end
  ...
  def sort_column
    Coupontype.column_names.include?(params[:sort]) ? params[:sort] : "name"
  end
